...
const populate = async () => {
  let u1 = await userCreate('hamkalo', 'p1');
  let u2 = await userCreate('azad', 'p2');
  let u3 = await userCreate('abaya', 'p3');
  let u4 = await userCreate('alia', 'p4');
  let u5 = await userCreate('sana', 'p5');
  let u6 = await userCreate('abhi3241', 'p6');
  let u7 = await userCreate('mackson3332', 'p7');
  let u8 = await userCreate('ihba001', 'p8');
  let u9 = await userCreate('Joji John', 'p9');
  let u10 = await userCreate('saltyPeter', 'p10');
  let u11 = await userCreate('monkeyABC', 'p11');
  let u12 = await userCreate('elephantCDE', 'p12');
  ...
  await questionCreate('Object storage for a web application', 'I am currently working on a website where, roughly 40 million documents and images should be served to its users. I need suggestions on which method is the most suitable for storing content with subject to these requirements.', [t5, t6], [a6, a7], u11, new Date('2023-02-18T01:02:15'), 200);
  await questionCreate('Quick question about storage on android', 'I would like to know the best way to go about storing an array on an android phone so that even when the app/activity ended the data remains', [t3, t4, t5], [a8], u12, new Date('2023-03-10T14:28:01'), 103);
  if(db) db.close();
  console.log('done');
}
